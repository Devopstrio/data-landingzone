...
import { ShieldCheck, Network, Database, Users, TrendingUp, BarChart4, Settings, LayoutDashboard, Globe, Zap, Box } from 'lucide-react';
      ...
      <div className="flex min-h-screen bg-slate-950 text-slate-100 font-sans">
        ...
        <aside className="w-72 bg-slate-900/40 backdrop-blur-3xl border-r border-slate-800 flex flex-col p-8 fixed h-full shadow-2xl">
          <div className="flex items-center gap-4 mb-12">
            ...
            <span className="text-xl font-black tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">Foundation IQ</span>
          </div>
          
          <nav className="flex-1 space-y-2">
            <NavItem to="/" icon={<LayoutDashboard size={20} />} label="Executive Summary" active />
            <NavItem to="/landingzones" icon={<Globe size={20} />} label="Landing Zone Hub" />
            <NavItem to="/onboarding" icon={<Zap size={20} />} label="Domain Onboarding" />
            <NavItem to="/governance" icon={<ShieldCheck size={20} />} label="Security & Policy" />
            <NavItem to="/costs" icon={<TrendingUp size={20} />} label="Cost Governance" />
            <NavItem to="/ownership" icon={<Users size={20} />} label="Domain Ownership" />
            <NavItem to="/platforms" icon={<Box size={20} />} label="Analytics Blueprints" />
          </nav>
        ...
        <main className="flex-1 ml-72">
          <header className="h-20 border-b border-slate-800 flex items-center justify-between px-10 bg-slate-950/50 backdrop-blur-md sticky top-0 z-10">
            <div className="flex items-center gap-2 text-slate-400 text-sm font-medium uppercase tracking-widest">
              ...
              <span>/</span>
              <span className="text-white font-bold">Data Landing Zone</span>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-sm font-bold text-white">Foundation Architect</p>
                <p className="text-[10px] text-blue-400 uppercase tracking-widest font-black">Global Data Platform</p>
              </div>
              <div className="w-10 h-10 bg-slate-800 rounded-full border border-slate-700 flex items-center justify-center font-bold text-slate-300">FA</div>
            </div>
          </header>
